...
namespace IndexDemoConsole
{
    partial class Program
    {
        ...
        private static void RunQueryInternal(DocumentClient client, string query, string collectionName)
        {
            Stopwatch sp = new Stopwatch();
            sp.Start();
            var resultsNum = client.CreateDocumentQuery<Document>(
                UriFactory.CreateDocumentCollectionUri(databaseId, collectionName),
                query);

            Console.WriteLine("\r\nQuery: {0} \r\nGot {1} document in {2} ms\r\n", query, resultsNum.AsEnumerable().Count(),
                sp.Elapsed.Milliseconds);
        }
